...
\begin{frame}{Problem statement}
  \begin{columns}[t]
    \column{0.5\textwidth}
      \begin{block}{Task}
        Determine the response of $N_s$ \emph{dynamical} radiators in response to a prescribed incident field
      \end{block}
    \column{0.5\textwidth}
    \begin{center}
...
\begin{frame}{Spatial acceleration: uniform grid}
  \begin{columns}
    \column{0.75\textwidth}
      ...
      \end{block}
      \begin{itemize}
        \item Physics imposes translational invariance: $\mathfrak{g}(\vb{r}_\text{obs}, \vb{r}_\text{src}) = \mathfrak{g}(\vb{r}_\text{obs} - \vb{r}_\text{src})$
        \item Matrix with \emph{structure} (Toeplitz), efficiently diagonalized by \alert{FFT}
      \end{itemize}
