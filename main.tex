...
\begin{frame}{The computational electromagnetics toolbox}
  ...
  \begin{alertblock}{Question}
    Can we take advantage of sparser discretization while simultaneously reducing overall cost complexity?
  \end{alertblock}
\end{frame}
...
\begin{frame}{More perspective}
  \begin{itemize}
    ...
    \item Time required: $\mathcal{O}(N_s^3) \to \SI{1.26}{\year}$ (at \SI{25}{\giga\flops})
  \end{itemize}
  \begin{block}{Goal}
    \begin{center}
      Develop methods that scale as $\mathcal{O}(N_t N_s \log^\alpha N_s)$ (years to minutes).
    \end{center}
    \begin{itemize}
...
\begin{frame}{Problem statement}
  \begin{columns}[t]
    ...
    \column{0.5\textwidth}
    \begin{center}
      \begin{tikzpicture}[>=latex, scale=1.5, rotate=-90]
        ...
        \draw[thick, snake it,] (0.5, -1.0) -- (0.5, -0.15);
        \draw[thick, snake it,] (0.5, 2.15) -- (0.5, 3.0);
      \end{tikzpicture}
    \end{center}
  \end{columns}
...
\begin{frame}{Spatial acceleration: uniform grid}
  \begin{columns}
    \column{0.75\textwidth}
      \begin{block}{Consider an easier problem (without dynamics)}
        \begin{equation*}
          \vb{E}\qty(\vb{r}_\text{obs}) = \sum_\text{src} \mathfrak{g}\qty(\vb{r}_\text{obs}, \vb{r}_\text{src}) \vb{J}\qty(\vb{r}_\text{src})
        \end{equation*}
      \end{block}
      \begin{itemize}
        ...
        \item Matrix with \emph{structure} (Toeplitz), diagonalized by Fourier transform (done quickly with \alert{FFT})
      \end{itemize}
  ...
  \begin{columns}
    \column{0.75\textwidth}
      ...
      \begin{itemize}
        ...
        \item Straightforward extension to time-dependent systems, 3D
      \end{itemize}
...
\begin{frame}{Nonlinear quantum optics}
  \begin{itemize}
    \item Liouville equation (ODE) couples $\vb{E}$ to internal degrees-of-freedom
      \begin{itemize}
        \item So-called ``density matrix'', $\hat{\rho}$
      \end{itemize}
    \item Evolution characterizes $\vb{J}$ directly
    \item Designed to accommodate absorption, relaxation at long times
  ...
  \begin{block}{Typical simulation}
    \begin{itemize}
      \item 10k particles separated by $\sim \SI{2}{\nano\meter}$ in cylindrical, cubic geometries
      \item Looking for signatures of \alert{confinement}, \alert{superradiance} in $\hat{\rho}$.
    \end{itemize}
  \end{block}
\end{frame}
