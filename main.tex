...
\begin{frame}
  \vspace{0.5cm}
  %\only<1>{\input{figures/pair_density_stats}}
  ...
  \vphantom{
    \parbox{\linewidth}{
      ...
      \begin{block}{Imposed structure}
        $g(\vb{r}, \vb{r}') = \vb{g}(\abs{\vb{r} - \vb{r}'})$; employ FFT to diagonalize matrix, accelerate $\oper{Z}\cdot\oper{A}$.
      \end{block}
      \begin{center}
...
\begin{frame}
  ...
  \visible<2-> {
    \begin{block}{Source-to-grid, grid-to-grid, grid-to-source}
      \begin{equation*}
        \mathcal{Z}^\text{far}_{ij} = \sum_{\vb{v} \in C_j} \sum_{\vb{u} \in C_i} \Lambda_{j\vb{v}} \, g(\vb{v}, \vb{u}) \, \Lambda_{i\vb{u}}
      \end{equation*}
    \end{block}
  }
\end{frame}
...
\begin{frame}{Determination of the $\Lambda$s}
  ...
  \begin{columns}
    \column{0.7\textwidth}
      \begin{itemize}
        \item Grid discretization sufficient to recover $\omega_0$ spatial variation
